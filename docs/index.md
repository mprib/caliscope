...
The standard install (`uv pip install caliscope`) includes intrinsic and extrinsic camera calibration as importable Python functions. Add `uv pip install caliscope[tracking]` for ONNX-based neural pose tracking, or `uv pip install caliscope[gui]` to add the desktop interface and 3D visualization (which also includes tracking).
