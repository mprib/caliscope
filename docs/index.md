...
The workflow currently requires you to provide your own synchronized frames or to provide [a file](project_setup.md#frame_timestampscsv) that specifies the time at which each frame was read so that caliscope can time-align the frames itself. A companion project is currently in development ([multiwebcam](https://github.com/mprib/multiwebcam)) that can perform concurrent USB webcam video for use cases where differences in frame capture of a few hundredths of a second are tolerable.
