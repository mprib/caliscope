...
def xy_gap_fill(xy_base:pd.DataFrame, max_gap_size=3) -> pd.DataFrame:
    ...
    for (port, point_id), group in xy_base.groupby(['port', 'point_id']):
        ...
        merged['img_loc_x'] = merged['img_loc_x'].interpolate(method='linear', limit=max_gap_size).round().astype('Int64')
        merged['img_loc_y'] = merged['img_loc_y'].interpolate(method='linear', limit=max_gap_size).round().astype('Int64')
...
def xyz_gap_fill(xyz_base:pd.DataFrame, max_gap_size=3) -> pd.DataFrame:
    ...
    if "frame_index" in xyz_base.columns:
        index_key = "frame_index"
    else:
        index_key = "sync_index"

    # Loop through each combination of port and point_id
    for (port, point_id), group in xyz_base.groupby(['port', 'point_id']):
        # Sort by frame_index to ensure the data is in order
        group = group.sort_values(index_key)
