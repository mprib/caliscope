...
logging.basicConfig(filename="synchronizer.log", level=logging.DEBUG)



with open("all_frames.json",) as f:
...
def earliest_next_frame(ports, current_port_frame_indices, all_frames):
    next_frame_times = []
    for port in ports:
        next_index = current_port_frame_indices[port] + 1
        port_index_key = str(port) + "_" + str(next_index)
        next_frame_at = all_frames[port_index_key]["frame_time"]
...
for _ in range(100):
    ...
    cutoff_time = earliest_next_frame(ports, current_port_frame_indices, all_frames)
    next_layer = []
    for port in ports:
        ...
        if frame_data["frame_time"] < cutoff_time:
