...
class Chessboard:
    ...
    Used for intrinsic calibration (and potentially extrinsic if
    disambiguation spike succeeds).
    ...
    Attributes:
        ...
        square_size_cm: Physical size of each square in centimeters.
            Irrelevant for intrinsic calibration (only relative geometry matters).
            Required for extrinsic calibration to set world scale (gauge).
    """

    rows: int
    columns: int
    square_size_cm: float = 1.0
    ...
    def get_object_points(self) -> np.ndarray:
        ...
        Returns:
            ...
            Units are centimeters (matching square_size_cm).
        """
        ...
        object_points[:, :2] = np.mgrid[0 : self.columns, 0 : self.rows].T.reshape(-1, 2) * self.square_size_cm
