...
class TestCameraSynthesizer:
    ...
    def test_single_ring_creates_correct_camera_count(self) -> None:
        array = CameraSynthesizer().add_ring(n=4, radius=1.0).build()
    ...
    def test_two_rings_accumulate_ports(self) -> None:
        array = CameraSynthesizer().add_ring(n=4, radius=2.0, height=0.0).add_ring(n=3, radius=2.0, height=0.5).build()
        assert len(array.cameras) == 7
        assert list(array.cameras.keys()) == [0, 1, 2, 3, 4, 5, 6]

    def test_drop_cam_ids_creates_gaps(self) -> None:
        array = CameraSynthesizer().add_ring(n=4, radius=2.0).drop_cam_ids(1, 3).build()
    ...
    def test_angular_offset_rotates_ring(self) -> None:
        ...
        array_no_offset = CameraSynthesizer().add_ring(n=4, radius=1.0).build()
        array_with_offset = CameraSynthesizer().add_ring(n=4, radius=1.0, angular_offset_deg=45).build()
        ...
        assert not np.allclose(pos0_no_offset, pos0_with_offset, atol=0.001)
    ...
    def test_rejects_insufficient_cameras(self) -> None:
        ...
        with pytest.raises(ValueError, match="at least 2 cameras"):
            CameraSynthesizer().add_ring(n=2, radius=1.0).drop_cam_ids(0, 1).build()
    ...
    def test_roll_variation_applies_random_roll(self) -> None:
        ...
        array1 = CameraSynthesizer().add_ring(n=4, radius=1.0, roll_variation_deg=10, random_seed=42).build()
        array2 = CameraSynthesizer().add_ring(n=4, radius=1.0, roll_variation_deg=10, random_seed=42).build()
    ...
    def test_different_seeds_give_different_results(self) -> None:
        array1 = CameraSynthesizer().add_ring(n=4, radius=1.0, roll_variation_deg=10, random_seed=42).build()
        array2 = CameraSynthesizer().add_ring(n=4, radius=1.0, roll_variation_deg=10, random_seed=99).build()
...
class TestAddLine:
    ...
    def test_creates_correct_camera_count(self) -> None:
        array = CameraSynthesizer().add_line(n=5, spacing=0.2).build()
        assert len(array.cameras) == 5

    def test_cameras_are_centered(self) -> None:
        """Line should be centered around x=0."""
        array = CameraSynthesizer().add_line(n=3, spacing=0.1).build()
        ...
        # Should be centered: -0.1, 0, 0.1
        assert np.isclose(sum(x_coords), 0, atol=0.001)
...
if __name__ == "__main__":
    ...
    # Basic ring
    ring = CameraSynthesizer().add_ring(n=4, radius=1.0, height=0.5).build()
    print(f"Ring: {len(ring.cameras)} cameras at cam_ids {list(ring.cameras.keys())}")

    # Two staggered rings
    double_ring = (
        CameraSynthesizer()
        .add_ring(n=4, radius=2.0, height=0.0)
    ...
    # With drops
    sparse = CameraSynthesizer().add_ring(n=6, radius=1.5).drop_cam_ids(1, 4).build()
    print(f"Sparse: {len(sparse.cameras)} cameras at cam_ids {list(sparse.cameras.keys())}")

    # Line with curvature
    line = CameraSynthesizer().add_line(n=5, spacing=0.3, curvature=1.0).build()
