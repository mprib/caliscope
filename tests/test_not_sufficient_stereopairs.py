...
def test_calibration_workflow(tmp_path: Path):
    ...
    rmse_initial = capture_volume.reprojection_report.overall_rmse
    assert rmse_initial > 0
    per_camera_rmse = capture_volume.reprojection_report.by_camera
    assert all(cam_id in per_camera_rmse for cam_id in capture_volume.camera_array.posed_cameras.keys())
    ...
    optimized = capture_volume.optimize(ftol=1e-3)

    # Log post-bundle adjustment RMSE and improvement
    rmse_post_bundle_adj = optimized.reprojection_report.overall_rmse
